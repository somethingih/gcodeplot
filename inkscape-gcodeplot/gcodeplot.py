...
LEGACY_INIT_CODE = "G00 S1; endstops|" \
                   "G00 E0; no extrusion|" \
                   "G01 S1; endstops|" \
                   "G01 E0; no extrusion|" \
                   "G21; millimeters|" \
                   ...
                   "G28 Y; home|" \
                   "G28 Z; home"
LEGACY_END_CODE = None

ENDER3V2_INIT_CODE = "G21; millimeters|" \
...
class Plotter(object):
    def __init__(self, xyMin=(7,8), xyMax=(204,178),
            ...
            initCode=LEGACY_INIT_CODE,
            endCode=LEGACY_END_CODE):
        self.xyMin = xyMin
        self.xyMax = xyMax
...
def getMachineProfileDefaults(machineProfile):
    normalized = re.sub(r'[^a-z0-9]+', '', machineProfile.lower())
    if normalized in ('ender3v2',):
        return {
            'name': 'ender3v2',
            'xyMin': (0, 0),
            'xyMax': (220, 220),
            'gcodePause': 'M0',
            'initCode': ENDER3V2_INIT_CODE,
            'endCode': ENDER3V2_END_CODE,
            'minPathLength': 0.30,
            'curveSmoothing': 0.35,
        }
    ...
    raise ValueError("Unknown machine profile " + machineProfile + ". Use ender3v2 or legacy.")
...
def segmentLength(segment):
    if len(segment) < 2:
        return 0.0
    return sum(math.hypot(segment[i][0]-segment[i-1][0], segment[i][1]-segment[i-1][1]) for i in range(1, len(segment)))

def pointDistance(a, b):
    return math.hypot(a[0]-b[0], a[1]-b[1])

def pointLineDistance(point, lineStart, lineEnd):
    dx = lineEnd[0] - lineStart[0]
    dy = lineEnd[1] - lineStart[1]
    denom = dx*dx + dy*dy
    if denom <= 1e-18:
        return pointDistance(point, lineStart)
    t = ((point[0]-lineStart[0])*dx + (point[1]-lineStart[1])*dy) / denom
    if t < 0:
        nearest = lineStart
    elif t > 1:
        nearest = lineEnd
    else:
        nearest = (lineStart[0] + t*dx, lineStart[1] + t*dy)
    return pointDistance(point, nearest)

def simplifySegmentRDP(segment, epsilon):
    if len(segment) <= 2 or epsilon <= 0:
        return list(segment)
    ...
    while stack:
        ...
        for i in range(start+1, end):
            d = pointLineDistance(segment[i], a, b)
            if d > maxDistance:
                maxDistance = d
                maxIndex = i
        if maxIndex is not None and maxDistance > epsilon:
            keep.add(maxIndex)
            stack.append((start, maxIndex))
            stack.append((maxIndex, end))
    return [segment[i] for i in sorted(keep)]

def chaikinOpen(segment):
    if len(segment) < 3:
        return list(segment)
    out = [segment[0]]
...
def smoothSegment(segment, curveSmoothing):
    if curveSmoothing <= 0 or len(segment) < 3:
        return list(segment)

    closed = segment[0] == segment[-1]
    base = list(segment[:-1] if closed else segment)
    if len(base) < 3:
        return list(segment)
    if segmentLength(base) < 2.0*curveSmoothing:
        return list(segment)

    if curveSmoothing < 0.75:
        passes = 1
    elif curveSmoothing < 1.5:
        passes = 2
    else:
        passes = 3
    ...
    for _ in range(passes):
        work = chaikinOpen(work)

    simplified = simplifySegmentRDP(work, curveSmoothing*0.30)
    if len(simplified) < 2:
        simplified = work
...
def smoothPaths(data, curveSmoothing):
    if curveSmoothing <= 0:
        return data, 0
    ...
    for pen in data:
        outSegments = []
        for segment in data[pen]:
            smoothed = smoothSegment(segment, curveSmoothing)
            if smoothed != list(segment):
                changed += 1
            outSegments.append(smoothed)
        if outSegments:
            outData[pen] = outSegments

    return outData, changed

def filterShortPaths(data, minPathLength):
    if minPathLength <= 0:
        # Still remove degenerate one-point segments to avoid pointless travel moves.
        minPathLength = 0
    ...
    for pen in data:
        ...
        for segment in data[pen]:
            if len(segment) < 2:
                removed += 1
                continue
            if segmentLength(segment) >= minPathLength:
...
if __name__ == '__main__':
 ...
 -d|--sort*: sort paths from inside to outside for cutting [default off]
    --curve-smoothing=x: round and simplify path corners by x mm (0 disables) [default 0.35 in ender3v2 profile]
    --min-path-length=x: ignore tiny path fragments shorter than x mm [default 0.3 in ender3v2 profile]
...
""")
    ...
    extractColor = None
    gcodePause = "@pause"
    machineProfile = "ender3v2"
    explicitArea = False
    explicitGcodePause = False
    explicitInitCode = False
    explicitEndCode = False
    explicitCurveSmoothing = False
    explicitMinPathLength = False
    optimizationTime = 30
    curveSmoothing = 0.0
    minPathLength = 0.0
    dpi = (1016., 1016.)
    ...
    try:
        ...
        while i < len(opts):
            ...
            if opt in ('-r', '--allow-repeats'):
            ...
            elif opt == '--curve-smoothing':
                curveSmoothing = max(0.0, float(arg))
                explicitCurveSmoothing = True
            elif opt == '--min-path-length':
                minPathLength = max(0.0, float(arg))
                explicitMinPathLength = True
                ...
                explicitInitCode = True
            elif opt == '--end-code':
                plotter.endCode = maybeNone(arg)
                explicitEndCode = True
    ...
    try:
        ...
        machineProfile = machineDefaults['name']
    ...
    if not explicitArea:
        plotter.xyMin = machineDefaults['xyMin']
        plotter.xyMax = machineDefaults['xyMax']
    if not explicitGcodePause:
        gcodePause = machineDefaults['gcodePause']
    if not explicitInitCode:
        plotter.initCode = machineDefaults['initCode']
    if not explicitEndCode:
        plotter.endCode = machineDefaults['endCode']
    if not explicitCurveSmoothing:
        curveSmoothing = machineDefaults['curveSmoothing']
    if not explicitMinPathLength:
        minPathLength = machineDefaults['minPathLength']

    if doDump:
        print('machine-profile=' + machineProfile)
        ...
        print('curve-smoothing=%g' % (curveSmoothing))
        print('min-path-length=%g' % (minPathLength))
    ...
    penData, smoothedSegments = smoothPaths(penData, curveSmoothing)
    if smoothedSegments > 0 and not quiet:
        sys.stderr.write("Smoothed %d path segments (curve-smoothing %.3f mm).\n" % (smoothedSegments, curveSmoothing))

    penData, removedSegments = filterShortPaths(penData, minPathLength)
